...
<?php
    ...
    if (isset($_SESSION['user_id'])) {
        $doctorQuery = "SELECT users.*, procedures.*, CONCAT(users.salutation, ' ', users.firstName, ' ', users.lastName) as doctorName
                        ...
                        WHERE users.doctorId = " . (int)$_SESSION['user_id'];
    ...
    $sql = "SELECT appointments.*, CONCAT(users.salutation, ' ', users.firstName, ' ', users.lastName) as doctorName FROM appointments LEFT JOIN users ON appointments.doctorId = users.doctorId WHERE status='new' AND appointments.doctorId = " . (int)$_SESSION['user_id'];
